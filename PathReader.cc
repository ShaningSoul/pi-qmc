// $Id: PathReader.cc 141 2009-07-10 17:33:43Z saadAK $
...
//  if (mpi) MPI::COMM_WORLD.Bcast(&slice(0,0),npart*NDIM,MPI::DOUBLE,0);
  ...
  //if (!permutationsFlag) {
    ...
    //compare p amd perm
    for (int i=0; i<npart; ++i) {
       if(p[i] != perm[i])    std :: cout <<"ERROR :: clone "<<cloneID<<",  worker "<<workerID<<", discrepancy found for permutation pickup at part "<<i <<": p[i]="<<p[i]<<"perm[i]="<<perm[i]<<std :: endl<<std::flush;
    }
